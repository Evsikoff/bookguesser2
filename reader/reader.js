...
class Reader {
    ...
    #onRelocate({ detail }) {
        ...
        // Game mode: report book ready on first relocate (book is rendered at this point)
...
const url = fileParam ? `https://bookguesser2.vercel.app/api/book/${encodeURIComponent(fileParam)}` : params.get('url')
...
if (gameMode) {
    ...
    // Listen for navigation commands from the game
    window.addEventListener('message', e => {
        if (!e.data || typeof e.data !== 'object') return
        if (e.data.type === 'navigate' && typeof e.data.fraction === 'number') {
            try { globalThis.reader?.view?.goToFraction(e.data.fraction) } catch {}
        }
    })
}
